...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty($amount_err)){
        $sql= "INSERT INTO investments_user (amount, date_added, user_id, payment_method, proyect_id) VALUES (?,?,?, ?, ?)";
        if($stmt = mysqli_prepare($connection, $sql)){
            $timezone = date_default_timezone_get();
            $_proyect_id =2;
            $payment_method = 2;
            mysqli_stmt_bind_param($stmt ,"ibiii",$amount, $timezone, $_SESSION['id'], $payment_method, $_proyect_id);
